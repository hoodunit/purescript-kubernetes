...
-- | create a Secret
createNamespaced :: Config -> String -> CoreV1.Secret -> Aff (Either MetaV1.Status CoreV1.Secret)
...
listForAllNamespaces :: Config -> Aff (Either MetaV1.Status CoreV1.SecretList)
...
-- | list or watch objects of kind Secret
listNamespaced :: Config -> String -> ListNamespacedOptions -> Aff (Either MetaV1.Status CoreV1.SecretList)
...
readNamespaced :: Config -> String -> String -> ReadNamespacedOptions -> Aff (Either MetaV1.Status CoreV1.Secret)
readNamespaced cfg namespace name options = Client.makeRequest (Client.get cfg url Nothing)
  where
    url = "/api/v1/namespaces/" <> namespace <> "/secrets/" <> name <> "" <> Client.formatQueryString options

-- | replace the specified Secret
replaceNamespaced :: Config -> String -> String -> CoreV1.Secret -> Aff (Either MetaV1.Status CoreV1.Secret)
