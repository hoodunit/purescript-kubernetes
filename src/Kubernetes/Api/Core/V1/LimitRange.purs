module Kubernetes.Api.Core.V1.LimitRange where
...
-- | create a LimitRange
createNamespaced :: Config -> String -> CoreV1.LimitRange -> Aff (Either MetaV1.Status CoreV1.LimitRange)
...
listForAllNamespaces :: Config -> Aff (Either MetaV1.Status CoreV1.LimitRangeList)
...
-- | list or watch objects of kind LimitRange
listNamespaced :: Config -> String -> ListNamespacedOptions -> Aff (Either MetaV1.Status CoreV1.LimitRangeList)
...
readNamespaced :: Config -> String -> String -> ReadNamespacedOptions -> Aff (Either MetaV1.Status CoreV1.LimitRange)
readNamespaced cfg namespace name options = Client.makeRequest (Client.get cfg url Nothing)
  where
    url = "/api/v1/namespaces/" <> namespace <> "/limitranges/" <> name <> "" <> Client.formatQueryString options

-- | replace the specified LimitRange
replaceNamespaced :: Config -> String -> String -> CoreV1.LimitRange -> Aff (Either MetaV1.Status CoreV1.LimitRange)
