module Kubernetes.Api.Storage.V1.StorageClass where
...
import Kubernetes.Api.Storage.V1 as StorageV1

-- | create a StorageClass
create :: forall e. Config -> StorageV1.StorageClass -> Aff (http :: HTTP | e) (Either MetaV1.Status StorageV1.StorageClass)
...
delete cfg body options = Client.makeRequest (Client.delete cfg url (Just encodedBody))
  where
    url = "/apis/storage.k8s.io/v1/storageclasses/{name}" <> Client.formatQueryString options
    encodedBody = encodeJSON body
...
deleteCollection cfg options = Client.makeRequest (Client.delete cfg url Nothing)
  where
    url = "/apis/storage.k8s.io/v1/storageclasses" <> Client.formatQueryString options
...
list :: forall e. Config -> ListOptions -> Aff (http :: HTTP | e) (Either MetaV1.Status StorageV1.StorageClassList)
...
read :: forall e. Config -> ReadOptions -> Aff (http :: HTTP | e) (Either MetaV1.Status StorageV1.StorageClass)
read cfg options = Client.makeRequest (Client.get cfg url Nothing)
  where
    url = "/apis/storage.k8s.io/v1/storageclasses/{name}" <> Client.formatQueryString options

-- | replace the specified StorageClass
replace :: forall e. Config -> StorageV1.StorageClass -> Aff (http :: HTTP | e) (Either MetaV1.Status StorageV1.StorageClass)
