module Kubernetes.Api.RbacV1.ClusterRole where
...
createClusterRole :: forall e. Config -> RbacV1.ClusterRole -> Aff (http :: HTTP | e) (Either MetaV1.Status RbacV1.ClusterRole)
...
listClusterRole :: forall e. Config -> ListClusterRoleOptions -> Aff (http :: HTTP | e) (Either MetaV1.Status RbacV1.ClusterRoleList)
listClusterRole cfg options = makeRequest (get cfg url Nothing)
  where
    url = "/apis/rbac.authorization.k8s.io/v1/clusterroles" <> formatQueryString options

-- | read the specified ClusterRole
readClusterRole :: forall e. Config -> Aff (http :: HTTP | e) (Either MetaV1.Status RbacV1.ClusterRole)
readClusterRole cfg = makeRequest (get cfg url Nothing)
  where
    url = "/apis/rbac.authorization.k8s.io/v1/clusterroles/{name}"

-- | replace the specified ClusterRole
replaceClusterRole :: forall e. Config -> RbacV1.ClusterRole -> Aff (http :: HTTP | e) (Either MetaV1.Status RbacV1.ClusterRole)
